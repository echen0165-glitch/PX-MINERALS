...
async function createTicket() { const subject = $('#ticket-subject').value.trim(), message = $('#ticket-message').value.trim(); if (!subject || !message) { alert('Veuillez saisir un sujet et un message.'); return; } try { const data = await request('/api/support', false, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subject, message }) }); alert(`Ticket ${data.ticketNumber} créé.`); window.location.reload(); } catch (error) { alert(error.message ?? 'Impossible de créer le ticket.'); } }
