...
async function copyText(value) { try { await navigator.clipboard.writeText(value); } catch { const area = document.createElement('textarea'); area.value = value; area.style.position = 'fixed'; area.style.opacity = '0'; document.body.appendChild(area); area.select(); document.execCommand('copy'); area.remove(); } }

async function request(url, retried = false, options = {}) {
  let response;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 12000);
  try { response = await fetch(url, { ...options, credentials: 'same-origin', signal: controller.signal }); } catch (error) { if (!retried) { await new Promise((resolve) => setTimeout(resolve, 700)); return request(url, true, options); } throw new Error(error.name === 'AbortError' ? 'Le serveur met trop de temps à répondre. Réessayez.' : 'Connexion momentanément indisponible. Réessayez dans un instant.'); } finally { clearTimeout(timeout); }
  if (response.status === 401 && !retried) { await new Promise((resolve) => setTimeout(resolve, 900)); return request(url, true); }
  if (response.status === 401) { window.location.replace('/app/login.html'); throw new Error('Votre session a expiré. Reconnectez-vous pour continuer.'); }
...
function mineralCard(item, progress) { const name = escape(item.mineral_name); const term = item.term === 'short' ? 'Court terme' : 'Long terme'; const active = item.status === 'active'; return `<article class="mineral-investment-card ${active ? 'is-working' : 'is-complete'}"><div class="mineral-art mineral-${String(item.mineral_name).toLowerCase().replace(/[^a-z]/g,'')}"><span class="mineral-rock">◆</span><b>${name}</b><small>RESSOURCE STRATÉGIQUE</small></div><div class="mineral-summary"><div class="mineral-title"><div><h3>${name} <span>${term}</span></h3><p>${active ? 'Votre investissement est actuellement en production.' : 'Cycle d’investissement terminé.'}</p></div><strong class="investment-status"><i></i>${active ? 'ACTIF' : 'TERMINÉ'}</strong></div><div class="mineral-stats"><div><i>▣</i><span>Montant investi</span><b>${formatMoney(item.price_xof)}</b></div><div><i>▥</i><span>Gain par jour</span><b>${formatMoney(item.daily_gain_xof)}</b></div><div><i>▤</i><span>Durée</span><b>${item.duration_days} jours</b></div><div><i>✦</i><span>Gains reçus</span><b>${formatMoney(item.gains_received_xof)}</b></div></div><div class="mineral-progress"><div><i style="width:${progress}%"></i></div><b>${Math.floor(progress)} %</b></div><div class="mineral-dates"><span><i>▤</i>Acheté le <b>${formatDate(item.purchased_at)}</b></span><span><i>▤</i>Fin prévue <b>${formatDate(item.ends_at)}</b></span></div>${miningAnimation(item, progress)}</div></article>`; }
...
async function createTicket() { const subject = $('#ticket-subject').value.trim() || 'Conversation avec le service client', message = $('#ticket-message').value.trim(); if (!message) { alert('Écrivez votre message.'); return; } try { const data = await request('/api/support', false, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subject, message }) }); alert(`Message envoyé au service client (${data.ticketNumber}).`); window.location.reload(); } catch (error) { alert(error.message ?? 'Impossible d’envoyer le message.'); } }
...
$('#copy-referral-page').addEventListener('click', async () => { if (!referralData?.code) return; await copyText(referralData.code); $('#copy-referral-page').textContent = 'Copié ✓'; setTimeout(() => { $('#copy-referral-page').textContent = 'Copier'; }, 1800); });
$('#copy-referral-link').addEventListener('click', async () => { if (!referralData?.code) return; await copyText(`${window.location.origin}/app/register.html?ref=${encodeURIComponent(referralData.code)}`); $('#copy-referral-link').textContent = 'Lien copié ✓'; setTimeout(() => { $('#copy-referral-link').textContent = 'Copier le lien'; }, 1800); });
$('#share-referral-link').addEventListener('click', async () => { if (!referralData?.code) return; const link = `${window.location.origin}/app/register.html?ref=${encodeURIComponent(referralData.code)}`; if (navigator.share) { await navigator.share({ title: 'PX MINERALS', text: 'Rejoignez PX MINERALS avec mon lien de parrainage.', url: link }); } else { await copyText(link); alert('Lien copié. Vous pouvez maintenant le partager.'); } });
...
setInterval(() => { if (selectedTicketId && !document.querySelector('#ticket-conversation.hidden')) openTicket(selectedTicketId).catch(() => {}); }, 12000);
init();
